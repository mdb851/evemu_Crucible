...
Next compile-first probe (twelfth): keep the **full probe-eleven** TU set; add **`character/CertificateMgrDB.cpp`** first (matches the new head), rebuild **`eve-test`**, first **`ld`** cluster only, revert on failure — add **`fleet/FleetService.cpp`**, **`ConsoleCommands.cpp`**, **`effects/EffectsDataMgr.cpp`**, **`effects/EffectsProcessor.cpp`**, **`inventory/ItemDB.cpp`**, etc., only when the next tail forces them.
