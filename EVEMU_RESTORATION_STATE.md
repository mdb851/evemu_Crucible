...
Next compile-first probe (nineteenth): keep the **full probe-eighteen** TU set; add **`system/SystemManager.cpp`** only first (new head is **`SystemManager::*`**), rebuild **`eve-test`**, first **`ld`** cluster only, revert on failure — still **defer** **`system/TargetManager.cpp`**, **`system/cosmicMgrs/CivilianMgr.cpp`**, **`system/BubbleManager.cpp`**, **`missions/MissionDataMgr.cpp`**, **`map/MapDB.cpp`**, **`market/MarketMgr.cpp`**, etc., until the next tail forces them.
