...
Next compile-first probe (fifth): same seven TU adds as probe four (third set + inventory/Inventory.cpp + InventoryItem.cpp + ItemFactory.cpp), plus Client.cpp; rebuild eve-test, capture first ld tail, revert on failure — probe four’s first cluster led with Client::SendNotifyMsg (then ItemData ctor, StaticDataMgr, AttributeMap).
