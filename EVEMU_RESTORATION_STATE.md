...
Next compile-first probe (thirty-fourth): keep the **full probe-thirty-three** TU set; add **`ship/Ship.cpp`** only first — defines **`ShipItem::Spawn`** (first symbol at the new head).

**Autopilot session (May 11–12, 2026):** Ran probes **26–33** in **`evemu_Crucible_github_work`** (Docker image **`evemu_app_build:latest`**: **`cmake -S . -B build-test -DEVEMU_BUILD_TESTS=ON -DCMAKE_BUILD_TYPE=Release`** + **`cmake --build build-test --parallel 4 --target eve-test`**). **`EVEMU_RESTORATION_STATE.md`** updated per probe; **`CMakeLists.txt`** remains the **5-TU** committed testlib after each revert. **`eve-test`** did **not** link successfully — **no `ctest`** run. Full link logs (untracked): **`_probe32_link_full.log`**, **`_probe33_link_full.log`**.
