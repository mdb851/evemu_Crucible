...
Next compile-first probe (twenty-sixth): keep the **full probe-twenty-five** TU set (probe-twenty-four plus **`market/MarketMgr.cpp`**); add **`system/cosmicMgrs/WormholeMgr.cpp`** only first — the new first missing symbol at the **`EntityList::Process()`** head is **`WormholeMgr::Process()`**. Rebuild **`eve-test`**, capture the first **`ld`** cluster, revert on failure. Still **defer** **`market/MarketBotMgr.cpp`** until the next tail forces it.
