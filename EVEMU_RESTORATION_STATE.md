...
**Frozen for validation-prep as of:** 2026-05-13. **Release-candidate code baseline:** **`9ddbfac309db942ba8dd493ecb61aa235f0f579c`** — includes **`823dae9f`** CreateContract traded-item rollback/null-guards, **`a35a6417`** courier **volume** column **UPDATE** rollback, **`9ddbfac3`** Tier-1 **`AcceptContract` / `CompleteContract`** hardening. **Branch tip** may include **doc-only** commits afterward; confirm with **`git rev-parse HEAD`** on **`restoration/contract-accept-corp`**. **Policy:** no further **code** churn except a **release-critical** defect found in validation; otherwise **live proof**, **design/feature** backlog, or **legacy-data** ops only.
...
**Status:** **Release-candidate freeze** — **May 13, 2026** — all listed fixes **present on branch**; **code baseline** **`9ddbfac309db942ba8dd493ecb61aa235f0f579c`**. **Code-side** work paused except **release-critical** validation finds. `docker compose build server` **PASS** at HEAD (prior session). **Isolated Docker stack** verified **May 13, 2026:** `docker compose -f docker-compose.isolated.yml -p evemu_iso build server` **PASS**; `up -d --force-recreate server` → **`evemu_isolated_server`** log **EVEmu Server is Online**. **Live** contract / market / corp matrix still **operator-owned** (see **Release validation checklist** above). Host **26100** / **26101** → container **26000** / **26001** when default **26000**–**26001** is taken.
...
- **`DeleteContract`:** items are **`ChangeOwner`** back to the **persisted `issuerCorpID`** when **`forCorp`** (not the session’s current corporation), matching listed corp ownership; **only the persisted `issuerID` (creating character)** may delete the row (returns **`false`** otherwise / missing contract). **Outstanding courier (`status == 0`) with `reward > 0`:** restores **personal** issuer wallet via **`AddBalance(+reward)`**; **corp** issuer receives **`corpSCC → issuerCorpID`** on the persisted **`issuerWalletKey`** division (**`ContractCollateralRefund`**), pairing corp reward escrow at create.
- **`CreateContract` (courier, `forCorp`, reward > 0):** validate **corp division** balance (`issuerWalletKey` / `GetCorpAccountKey()`); after insert, **`TransferFunds(issuer corp → corpSCC, …, ContractCollateral)`** escrows reward (same SCC pool pattern as corp collateral). **`CompleteContract` (courier success):** when **`issuerForCorp`**, pay reward **`from corpSCC`** with **`Account::KeyType::Cash`** so completion does not debit the issuer division a second time. **`CompleteContract` (courier fail, status 7):** when **`issuerForCorp`** and **`contractType == 3`**, **`TransferFunds(corpSCC → issuer corp division, ContractCollateralRefund)`** returns escrowed reward (mirrors outstanding **`DeleteContract`** refund).
- **`AcceptContract` (item exchange, personal issuer, reward > 0):** reward sufficiency uses **`AccountDB::GetCharacterISKBalance(issuerID)`** instead of **`GetCharacterRef(issuerID)->balance(...)`**, avoiding **null / unloaded issuer** undefined behavior while matching **`chrCharacters.balance`**.
- **`AcceptContract` / `CompleteContract` (Tier 1 pass):** item-exchange **start station** + **`GetMyInventory`** null-guard; requested-stack **`Split`** null-guard; **courier accept** null-safe **`GetSolarSystemRef`** naming, **`SpawnItem`** failure rolls back **collateral**, skip **`itemID==0`** in crate fill; **courier complete** wrong-station message uses null-safe **`GetStationRef`**; handoff loop null-checks **`GetItemRef`** before **`ChangeOwner`/`Move`**. (Item-exchange **FindRequestStack** / **`GetByID`** / traded **`GetItemRef`** guards + **inventory-before-ISK** unchanged from prior slice.)
- **`CreateContract` (traded items):** null-safe courier volume path; **`RollbackCreateContractAfterInsert`** on post-insert failures (DB query / factory load / **`ctrItems`** insert / empty item list) reverses courier reward debits and removes orphan **`ctrContracts`** row. **Courier `volume` UPDATE** failure now triggers the same rollback (no stuck partial courier row).
- **`CertificateMgrService::GetCertificatesByCharacter`:** no longer dereferences a null **`GetCharacterRef`** when the character is not loaded — returns an **empty certificate rowset**.
- **`DogmaIMBound::LoadAmmoToModules` / `LoadAmmoToBank`:** module / master **`GetItemRef`** is null-checked before **`->flag()`** (commit: **`fix: null-check DogmaIM module GetItemRef before flag()`**); missing charge stack throws **`CantFindChargeToAdd`** before **`LoadCharge`** on the non-linked bank path (**`LoadAmmoToModules`** always).
- **`ContractProxy::CollectMyPageInfo`:** first DB failure log refers to **`outstandingContractsQuery`**, not **`mainQuery`** (misleading ops text only).
...
**Documented gaps (code audit, no safe server change without design):** `PlaceBid` / `FinishAuction` appear only in **commented** client snippets in `ContractProxy.cpp` — **no bound RPC handlers**. `GetMyExpiredContractList` returns **`nullptr`**. No background job marks **`ctrContracts`** expired or refunds wallet symmetry beyond **`DeleteContract`** / **`CompleteContract`** paths already hardened. `CompleteContract` handles completion status **4** and **7** only; other client status codes need **live trace**.
