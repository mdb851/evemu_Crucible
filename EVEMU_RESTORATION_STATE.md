...
Next compile-first probe (twenty-third): keep the **full probe-twenty-two** TU set (probe-twenty-one plus **`system/BubbleManager.cpp`**); add **`missions/MissionDataMgr.cpp`** only first — the new first missing symbol at the **`EntityList::Process()`** head is **`MissionDataMgr::Process()`**. Rebuild **`eve-test`**, capture the first **`ld`** cluster, revert on failure. Still **defer** **`map/MapDB.cpp`**, **`market/MarketMgr.cpp`**, **`system/cosmicMgrs/WormholeMgr.cpp`**, **`market/MarketBotMgr.cpp`** until the next tail forces them.
