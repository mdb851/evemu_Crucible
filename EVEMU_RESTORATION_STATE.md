...
Next compile-first probe (thirty-second): keep the **full probe-thirty-one** TU set; add **`station/StationDB.cpp`** only first — owns **`StationDB::LoadOffices`** (first symbol at the new head).
