...
Next compile-first probe: keep ContractProxy.cpp + AccountService.cpp + Character.cpp + ClientSession.cpp, add inventory/Inventory.cpp + inventory/InventoryItem.cpp + inventory/ItemFactory.cpp, rebuild eve-test, capture first ld tail, revert on failure; only add Client.cpp next if Client::SendNotifyMsg remains in the first blocker cluster.
