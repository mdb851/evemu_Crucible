...
Next compile-first probe (fourteenth): keep the **full probe-thirteen** TU set; add **`ConsoleCommands.cpp`** only, rebuild **`eve-test`**, first **`ld`** cluster only, revert on failure — defer **`effects/EffectsDataMgr.cpp`**, **`effects/EffectsProcessor.cpp`**, **`inventory/ItemDB.cpp`** until the new tail forces them.
