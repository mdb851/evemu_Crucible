...
Next compile-first probe (sixth): extend probe five’s nine TU adds with the smallest set that covers the new first missing symbols (e.g. **`inventory/AttributeMap.cpp`**, **`inventory/ItemType.cpp`** where **`ItemData`** ctors are defined — then re-link and let the next tail choose among **`EntityList.cpp`**, **`CorporationDB.cpp`**, **`CharacterDB.cpp`**).
