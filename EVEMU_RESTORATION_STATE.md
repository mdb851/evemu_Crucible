...
Next compile-first probe (twenty-seventh): keep the **full probe-twenty-six** TU set (probe-twenty-five plus **`system/cosmicMgrs/WormholeMgr.cpp`**); add **`market/MarketBotMgr.cpp`** only first — the new first missing symbol at the **`EntityList::Process()`** head is **`MarketBotMgr::Process(bool)`**.
