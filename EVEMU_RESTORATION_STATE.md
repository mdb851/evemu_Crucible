...
Next compile-first probe (fifth): keep probe four’s seven TU adds (ContractProxy, AccountService, Character, ClientSession, inventory/Inventory, InventoryItem, ItemFactory), plus **Client.cpp** and **StaticDataMgr.cpp** only; rebuild eve-test, stop at first new ld cluster, revert on failure. **Defer** EntityList.cpp, CorporationDB.cpp, CharacterDB.cpp until the new tail forces them — probe four showed inventory/item mostly resolved; first blockers are **Client::SendNotifyMsg** + **StaticDataMgr** + **ItemData** + **AttributeMap** (expect **Client.cpp** to widen the graph a lot).
