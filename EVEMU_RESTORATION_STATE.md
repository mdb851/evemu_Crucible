...
Next compile-first probe (twenty-second): keep the **full probe-twenty-one** TU set (probe-twenty plus **`system/cosmicMgrs/CivilianMgr.cpp`**); add **`system/BubbleManager.cpp`** only first — the new first missing symbol is **`BubbleManager::Process()`**. Rebuild **`eve-test`**, capture the first **`ld`** cluster, revert on failure. Still **defer** **`missions/MissionDataMgr.cpp`**, **`map/MapDB.cpp`**, **`market/MarketMgr.cpp`**, **`system/cosmicMgrs/WormholeMgr.cpp`**, **`market/MarketBotMgr.cpp`** until the next tail forces them.
