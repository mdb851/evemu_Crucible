...
**Frozen for validation-prep as of:** 2026-05-13. **HEAD (branch tip):** **`ebec0231`** — *release-candidate freeze documentation* (`.gitignore` **`build-test/`**, final completion matrix, operator validation checklist) on top of code **`9ddbfac309db942ba8dd493ecb61aa235f0f579c`** (includes **`823dae9f`** CreateContract traded-item rollback/null-guards, **`a35a6417`** courier **volume** column **UPDATE** rollback, **`9ddbfac3`** Tier-1 **`AcceptContract` / `CompleteContract`** hardening). **Policy:** no further code churn on this branch except a **release-critical** defect discovered during validation; otherwise advance **live proof**, **design/feature** backlog, or **legacy-data** ops only.
...
**Status:** **Release-candidate freeze** — **May 13, 2026** — all listed fixes **present on branch**; code baseline **`9ddbfac309db942ba8dd493ecb61aa235f0f579c`**; branch tip **`ebec0231`** records freeze doc + matrix + checklist. **Code-side** work paused except **release-critical** validation finds. `docker compose build server` **PASS** at HEAD (prior session). **Isolated Docker stack** verified **May 13, 2026:** `docker compose -f docker-compose.isolated.yml -p evemu_iso build server` **PASS**; `up -d --force-recreate server` → **`evemu_isolated_server`** log **EVEmu Server is Online**. **Live** contract / market / corp matrix still **operator-owned** (see **Release validation checklist** above). Host **26100** / **26101** → container **26000** / **26001** when default **26000**–**26001** is taken.
