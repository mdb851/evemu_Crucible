...
Next compile-first probe (twenty-fifth): keep the **full probe-twenty-four** TU set (probe-twenty-three plus **`map/MapDB.cpp`**); add **`market/MarketMgr.cpp`** only first — the new first missing symbol at the **`EntityList::Process()`** head is **`MarketMgr::Process()`**. Rebuild **`eve-test`**, capture the first **`ld`** cluster, revert on failure. Still **defer** **`system/cosmicMgrs/WormholeMgr.cpp`**, **`market/MarketBotMgr.cpp`** until the next tail forces them.
