...
Next compile-first probe (twentieth): keep the **full probe-nineteen** TU set (probe-eighteen plus **`system/SystemManager.cpp`**); add **`system/TargetManager.cpp`** only first — the new first missing symbol is **`TargetManager::Process()`**. Rebuild **`eve-test`**, capture the first **`ld`** cluster, revert on failure. Still **defer** **`system/cosmicMgrs/CivilianMgr.cpp`**, **`system/BubbleManager.cpp`**, **`missions/MissionDataMgr.cpp`**, **`map/MapDB.cpp`**, **`market/MarketMgr.cpp`**, **`system/cosmicMgrs/WormholeMgr.cpp`**, **`market/MarketBotMgr.cpp`** until the next tail forces them.
