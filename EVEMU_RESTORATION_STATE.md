...
Next compile-first probe (twenty-first): keep the **full probe-twenty** TU set (probe-nineteen plus **`system/TargetManager.cpp`**); add **`system/cosmicMgrs/CivilianMgr.cpp`** only first — the new first missing symbol is **`CivilianMgr::Process()`**. Rebuild **`eve-test`**, capture the first **`ld`** cluster, revert on failure. Still **defer** **`system/BubbleManager.cpp`**, **`missions/MissionDataMgr.cpp`**, **`map/MapDB.cpp`**, **`market/MarketMgr.cpp`**, **`system/cosmicMgrs/WormholeMgr.cpp`**, **`market/MarketBotMgr.cpp`** until the next tail forces them.
