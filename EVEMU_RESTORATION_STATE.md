...
Next compile-first probe (sixth): keep the **full probe-five set** (nine carried TUs + **`Client.cpp`** + **`StaticDataMgr.cpp`**), add **`inventory/AttributeMap.cpp`** and **`inventory/ItemType.cpp`** only, rebuild **`eve-test`**, capture first **`ld`** cluster, revert on failure. **Defer** **`EntityList.cpp`**, **`CorporationDB.cpp`**, **`CharacterDB.cpp`** until that new tail forces them — probe five cleared the **client / static-data** head; next wall is **item metadata + attributes**, then the tail will show whether **entity / corp / character DB** is still the next forced ring.
