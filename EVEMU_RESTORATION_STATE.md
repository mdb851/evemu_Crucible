...
Next compile-first probe (thirteenth): keep the **full probe-twelve** TU set; add **`fleet/FleetService.cpp`** first (first hard missing after cert ring), rebuild **`eve-test`**, first **`ld`** cluster only, revert on failure — add **`ConsoleCommands.cpp`** next if **`ConsoleCommand::*`** still leads; defer **`effects/EffectsDataMgr.cpp`**, **`effects/EffectsProcessor.cpp`**, **`inventory/ItemDB.cpp`** until the tail forces them (or split one effects TU if the tail isolates **`FxDataMgr`** vs **`FxProc`**).
