...
Next compile-first probe (twenty-eighth): keep the **full probe-twenty-seven** TU set; add **`station/Station.cpp`** only first — it defines **`StationItem::GetGuestList`** (first symbol family in the new head cluster). **`agents/Agent.cpp`** remains deferred until the next first-cluster head forces it.
